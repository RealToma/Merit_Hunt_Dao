...
const Navbar = () => {
    ...
    const [balanceHunt, setBalanceHunt] = useState(0);
    ...
    return (
        <StyledComponent>
            {active ? <BalanceBox>Balance: {balanceHunt} HUNT</BalanceBox> : <></>}
            <ConnectWalletBtn onClick={() => {
                ...
                {active ? account.slice(0, 6) + "..." + account.slice(-4) : <><FaWallet /><Box display={"flex"} ml={"10px"}>Connect</Box></>}
                {
                    flagDrop ?
